...
function goToWall() {
    // $.post("/js2py", JSON.stringify(filteredMarkets[activeMarketIndex]), window.location.href = "/marketwall", JSON)
    // better to use below "headers" method than above method so you don't have to mix the CSRF token in with the filtered markets data object
    $.ajax({
        url: "/js2py",
        method: "POST",
        headers: {'X-CSRFToken': getCookie("csrftoken")},
        data: JSON.stringify(filteredMarkets[activeMarketIndex]),
        dataType: "json",
        success: window.location.href = "/marketwall"
    })
}
